...
> Trasladado de `.agent/specs/propuestas/categorias-y-subcategorias.md` el
> 2026-08-29, cuando el humano aprobó promoverlo a feature con la recomendación
> del agente para SP1, SP2, SP3 y SP4 (ver § Huecos y preguntas al humano —
> quedan anotadas como RESUELTAS, no se borran). Los 15 criterios de este
> documento ya están en `.agent/features.json` bajo F-026 y pasan a `[ya]`.
>
...
> avanzados y reseñas. El pedido se partió en cuatro propuestas independientes;
> esta es la del selector de categorías. Las otras viven en F-025 (navegación),
> F-027 (filtros y orden) y `.agent/specs/propuestas/resenas-y-calificaciones.md`
> (todavía sin promover: el humano pidió revisarla antes).
>
> **Aviso que sigue siendo cierto tras la resolución de SP1:** la palabra
> «subcategorías» no tiene dato detrás en este sistema hoy. El humano resolvió
> construir un solo nivel ahora (§ Huecos y preguntas al humano, SP1) y dejar la
> subcategoría real como feature futuro.
...
6. **Ruta propia por categoría** (RESUELTO por SP2): `/[slug]/c/[categorySlug]`
   (por crear), estática y revalidada por `storeCatalogTag`.
7. **Un slug de categoría propio, único por negocio y congelado al crearse**
   (RESUELTO por SP3): migración aditiva `@@unique([businessId, slug])` en
   `LocalCategory` y desambiguación con sufijo si colisiona.

### Fuera (explícito)

- **Filtros por precio, por disponibilidad y ordenamientos.** Son F-027.
  RESUELTO por SP4: esta propuesta es dueña de la categoría como
  **navegación** (URL propia, cacheable, indexable); F-027 la reutiliza como
  **filtro** dentro de una categoría, sin redefinir el parámetro.
- **Botón de atrás y breadcrumb.** Son F-025. Esta propuesta **crea el nivel
  intermedio** que ese breadcrumb va a querer nombrar (tienda › categoría ›
  producto), así que le entrega el dato y no la pieza visual.
- **Reseñas y calificaciones.** Propuesta hermana
...
- **Crear, renombrar, ordenar o borrar categorías desde el panel.** Las
  ...
  su POS.
- **El segundo nivel (subcategorías) y poblar `GlobalCategory` en producción.**
  RESUELTO por SP1: fuera de este ciclo, feature futuro. Hoy no existe (I2) y
  F-021 ya dejó dicho que eso «sigue siendo un feature que abrirá el humano»
  (`.agent/specs/F-021/spec.md:62-64`). Aquí solo se **consume** lo que haya.
...
`GlobalCategory` del canónico, que hoy solo tiene datos en el seed (I2).
...
renombrarla** (E7): el slug se congela en el primer `CREATE` (RESUELTO por SP3).
Es el mismo criterio que ADR 0018 (a) ya fija para los slugs de tienda («un
valor no se reasigna nunca»).
...
el filtro por categoría **no puede ser un query param de `/[slug]`** (I6).
RESUELTO por SP2: ruta propia `/[slug]/c/[categorySlug]`.
...
**R14 — el contrato con cuadrecaja no cambia.** Con SP1 resuelto en (a), no
entra ni sale ningún campo por `/api/internal/*` y `docs/sync-contract.md` no
sube de versión.
...
| Dos categorías del mismo negocio cuyos nombres slugifican igual | Dos URLs distintas, cada una con sus productos (desambiguación con sufijo al crear, I3)                                                                      |
| Entrega desordenada de dos eventos `CATEGORY`                   | Puede quedar el nombre viejo: `handleCategory` no tiene guarda anti-rancia (I8). Se acepta por escrito; cerrarlo es del arquitecto si toca                   |
...
No lleva padre, no lleva orden, no lleva visibilidad.

Migración necesaria para este feature (SP3 resuelto en (a)):

| Dónde                                       | Qué                                                                                     |
| ------------------------------------------- | --------------------------------------------------------------------------------------- |
| `LocalCategory`                             | `@@unique([businessId, slug])` y desambiguación al crear                                |
| `src/features/sync/server/handlers/misc.ts` | Generar el slug sin colisión y no moverlo al renombrar (R8)                             |
| `docs/sync-contract.md`                     | Documentar el `payload` de `CATEGORY` (hoy no está) — deuda que F-022 ya promete cerrar |
...
Los 15 ya están en `.agent/features.json` bajo F-026 (marcados `[ya]`). Escritos
contra la forma resuelta en SP2 (una ruta propia por categoría).

1. `[ya]` `GET /tienda-demo` responde 200 y su HTML contiene un enlace por
...
2. `[ya]` `GET` de la vista de «Bebidas» de `tienda-demo` responde 200, su
...
8. `[ya]` Tras un `PRODUCT`/`UPDATE` que cambia `localCategoryId`, el producto
   no aparece en la vista de la categoría anterior y sí en la de la nueva, en la
   misma comprobación (E14).
9. `[ya]` Tras un `CATEGORY`/`DELETE`, la categoría desaparece del selector, su
   vista responde 404 y **los nombres de sus productos siguen apareciendo en
   `/[slug]`** (E8, I4).
10. `[ya]` Tras un `CATEGORY`/`UPDATE` que renombra la categoría, el selector
    muestra el nombre nuevo y la URL de la categoría sigue respondiendo 200 (E7,
    R8).
11. `[ya]` Dos categorías del mismo negocio cuyos nombres slugifican igual
    producen dos URLs distintas, y cada una lista solo sus propios productos
    (I3).
12. `[ya]` `npm run build` sigue marcando `/[slug]` como ● (SSG), no como ƒ
    (Dynamic) — el primer `acceptance_criteria` de F-004 sigue cierto (R10).
13. `[ya]` `node scripts/check-bundle-budget.mjs` termina con código 0, y
    `grep -rn "use client"` sobre los archivos nuevos de este feature no devuelve
    nada (R9).
14. `[ya]` Con el JavaScript deshabilitado, seguir un enlace del selector
    carga la vista de la categoría con sus productos en el HTML (E15).
15. `[ya]` `bash .agent/verify.sh F-026 --full` termina con código 0.
...
desarrollo ni en producción. RESUELTO por SP1: un solo nivel ahora.
...
«Refrescos» deja el slug `bebidas` para siempre. RESUELTO por SP3: migración
aditiva `@@unique([businessId, slug])`, slug congelado en el `CREATE`.
...
(SSG). RESUELTO por SP2: ruta aparte `/[slug]/c/[categorySlug]`.
...
`src/features/sync/schemas.ts:48-54`. F-022 ya promete cerrarlo en su tabla de
...
igual en cuanto el nombre es un elemento de navegación**. Preexistente; el
arquitecto decide si se cierra aquí o se acepta por escrito.
...
feature.» RESUELTO por SP4: navegación aquí, filtro en F-027.
...
ninguna? RESUELTO por el humano el 2026-08-29: opción (a).**
Un solo nivel, ahora. El selector lista las `LocalCategory` que la tienda tiene
en stock, tal cual las manda el POS. Cero cambios de contrato, cero espera, y
cubre el 100 % de los datos reales de hoy. La palabra «subcategoría» sale de
este ciclo y vuelve como feature propio cuando haya dato — vía `GlobalCategory`
con un camino de clasificación, o vía v5 del contrato si cuadrecaja confirma
que su `Categoria` tiene padre (pendiente: preguntarles).

**SP2 — ¿Dónde vive la vista por categoría, dado que `/[slug]` no puede leer
`searchParams` sin dejar de ser SSG (I6)? RESUELTO por el humano el
2026-08-29: opción (a).**
Ruta propia por categoría, `/[slug]/c/[categorySlug]` (por crear), estática y
revalidada por `storeCatalogTag`. Se lee sin JavaScript, se cachea en el CDN, se
comparte por enlace, se indexa (R12) y `/[slug]` sigue ● (SSG).

**SP3 — Dado SP2 = (a): ¿qué identifica a una categoría en la URL, dado que su
slug no es único ni estable (I3)? RESUELTO por el humano el 2026-08-29:
opción (a).**
Slug garantizado por este feature: `@@unique([businessId, slug])`,
desambiguación con sufijo al crear, y el slug **congelado** en el primer
`CREATE`. URLs legibles (`/tienda-demo/c/bebidas`). Mismo criterio que ADR 0018
(a) para el primer nivel.

**SP4 — ¿Dónde está la frontera entre esta propuesta y F-027? RESUELTO por
el humano el 2026-08-29: opción (a).**
Esta propuesta es dueña de la categoría como **navegación** —una vista por
categoría, enlazable y cacheable—; F-027 aplica sus filtros de precio,
disponibilidad y orden **dentro** de una categoría, reusando la lectura que se
define aquí, y reutiliza el mismo nombre de parámetro sin redefinirlo.
...
- **Si la vista por categoría se pagina.** Depende del tope que fije F-027 para
  el catálogo completo; hoy no hay paginación en `/[slug]`.
...
- **Qué hace el breadcrumb con este nivel.** Esta propuesta le entrega el dato
  (tienda › categoría › producto); la forma la decide F-025.
