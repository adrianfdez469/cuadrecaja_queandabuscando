...
- `bash .agent/verify.sh F-017 --full` → **0** en las nueve etapas
  ...
  `handlers/store.ts` en `progress/F-017.md` — mías, las arreglé).
...
[...]`, `expected ['bodega-dos'] to deeply equal [...]`); restauré con el
  ...
  - En vivo, contra un servidor real (`next dev` en el puerto 3100, Postgres
    de docker): con el arreglo de `handlers/store.ts` neutralizado
...
| `src/features/sync/server/handlers/types.ts`                                         | `HandlerOutcome.touchedSlugValues` pasa de `string[]` a `SlugTouchSet`                                                                                                                                                                                                                                         | Archivo de tipos compartido, no lógica de ningún escritor. `handlers/store.ts` no cambió una sola línea: su `siblingTouch()` ya devolvía `expandBrandTouch(...)` directamente, así que el tipo más estricto lo satisface sin tocarlo |
...
Reemplacé temporalmente el cuerpo de `siblingTouch()` en `handlers/store.ts`
...
undefined' is not assignable to type 'SlugTouchSet | undefined'`, en las
   dos líneas de `handlers/store.ts` donde se usa el resultado.
