...
export async function generateMetadata({
  ...
  if (result.totalCount === 0) {
    return { title: `Sin resultados para «${result.term}»`, robots: { index: false } };
  }
  const plural = result.totalCount === 1 ? "resultado" : "resultados";
  return { title: `${result.term} · ${result.totalCount} ${plural}`, robots: { index: false } };
}
...
export default async function StoreSearchPage({
  ...
  const [result, rates] = await Promise.all([
    loadSearch(resolution.storeId, term, page),
    getStoreRates(resolution),
  ]);

  // R13/E16: scheduled AFTER the response leaves, and never throws — a
  // failure here must not become the shopper's problem.
  after(() =>
    recordStoreSearchQuery({
        ...
        {empty ? (
          <>
            <h1 className="mt-8 text-2xl font-semibold break-words">
              Sin resultados para «{result.term}»
            </h1>
            <p className="text-fg-muted mt-2 max-w-2xl break-words">
              No encontramos ningún producto para «{result.term}» en {store.name}.
            </p>
            {wasTruncated(raw) && (
              <p className="text-fg-muted mt-2 text-sm">
          ...
          <>
            <h1 className="mt-8 text-2xl font-semibold break-words">
              Resultados para «{result.term}»
            </h1>
            <p className="text-fg mt-2">
              {result.totalCount} {result.totalCount === 1 ? "resultado" : "resultados"} en{" "}
              {store.name}.
            </p>
            {wasTruncated(raw) && (
            ...
            {outOfRange ? (
              <Alert tone="muted" className="mt-6">
                ...
                <Link
                  href={`/${store.canonicalSlug}/buscar?q=${encodeURIComponent(result.term)}`}
